...
def main():
    ...
    use_github = use_git and os.getcwd() == git_dir
    if use_github:
    ...
    for name in names:
        ...
        if is_dir:
            ...
            link = 'https://raw.githubusercontent.com/%s/%s/master/%s' % (github_user, github_repo, name)
            link_class = 'octicon-file-media' if is_media else 'octicon-file'
        elif use_git:
            link = '//git.oschina.net/phuslu/phuslu-github-io/raw/master/%s' % os.path.abspath(name)[len(git_dir)+1:]
            link_class = 'octicon-file-media' if is_media else 'octicon-file'
        else:
            link = name
            link_class = 'octicon-file-media' if is_media else 'octicon-file'
